...
CAS 不是 Java 自己实现的，是直接调用 CPU 的一条原子指令

CAS 是一种无锁的并发控制方式，全称 Compare And Swap，比较并交换。原理是操作一个值之前，先比较它是不是我期望的那个，是就改，不是就不改然后重试，整个比较和交换的动作由 CPU 保证是原子的。
Java 里的 AtomicInteger 这些原子类底层就是 CAS，比 synchronized 性能好，因为它不阻塞线程，失败了自旋重试就行。
但 CAS 也有缺点，一个是竞争激烈时自旋太久会消耗 CPU，另一个是 ABA 问题，就是值被改了又改回来，CAS 发现值没变以为没人动过，实际上已经被动过了。解决 ABA 问题可以用 AtomicStampedReference，加一个版本号，每次修改版本号加一，这样就能识别出来。
