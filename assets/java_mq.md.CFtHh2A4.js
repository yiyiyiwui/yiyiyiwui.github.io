...
<span class="line"><span> rabbitmq:3.8-management</span></span></code></pre></div><p>账号密码初始都是：guest 开发中咱们不会接触管理员用户，所以这里创建自己的用户 <img src="`+h+'" alt=""> 因为要用多个微服务，对应的也有数据库，为了更好区分，我们要隔离起来创建各自项目的虚拟主机 <img src="'+u+'" alt=""> 之后管理员给这个用户的虚拟主机分配权限</p><p>RabbitMQ消息队列主要分为两种，一种就类似于Feign，点对点模型。另一种是就是分布式的，发布/订阅模型 <img src="'+d+'" alt=""> 发布者发布消息后，我们通过交换机接收消息，然后传给队列，队列存储消息，然后再发给各个消费者，生产者能发多个消息，也可以有多个交换机接收，同样，交换机也可以发送多个消息给队列，队列也可以给多个消费者发送消息。</p><h2 id="问题-在mq中怎么和feign一样发送接收消息呢" tabindex="-1">问题：在MQ中怎么和Feign一样发送接收消息呢？ <a class="header-anchor" href="#问题-在mq中怎么和feign一样发送接收消息呢" aria-label="Permalink to “问题：在MQ中怎么和Feign一样发送接收消息呢？”">​</a></h2><p><img src="'+g+`" alt=""> 1：首先我们先在生产方建立连接工厂，创建连接对象和通道</p><div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark" style="--shiki-light:#24292e;--shiki-dark:#e1e4e8;--shiki-light-bg:#fff;--shiki-dark-bg:#24292e;" tabindex="0" dir="ltr"><code><span class="line"><span>public void testSendMessage() throws IOException, TimeoutException {</span></span>
