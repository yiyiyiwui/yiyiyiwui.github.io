import{_ as a,o as n,c as p,ah as e,bq as i,br as l,bs as t,bt as c,bu as r}from"./chunks/framework.Df4DMI_z.js";const y=JSON.parse('{"title":"SpringCloud","description":"","frontmatter":{"outline":"deep"},"headers":[],"relativePath":"java/springCloud.md","filePath":"java/springCloud.md","lastUpdated":1779160829000}'),d={name:"java/springCloud.md"};function o(h,s,g,b,u,k){return n(),p("div",null,[...s[0]||(s[0]=[e('<h1 id="springcloud" tabindex="-1">SpringCloud <a class="header-anchor" href="#springcloud" aria-label="Permalink to “SpringCloud”">​</a></h1><blockquote><p>负载均衡、Nacos、Feign、熔断器、网关、断言</p></blockquote><h2 id="学前了解" tabindex="-1">学前了解 <a class="header-anchor" href="#学前了解" aria-label="Permalink to “学前了解”">​</a></h2><p>单体应用架构是把所有应用模块放一起，用的时候都去数据库调用，如果数据库崩了，就会影响所有的模块运行。</p><p>微服务架构是把应用模块分成好几个各自的模块，每个模块都有各自的数据库，用的时候互相调用即可，如果有一个模块崩了，不用影响其他模块运行，大大提高了容错率。 <img src="'+i+'" alt=""></p><p>有了思想，就需要有技术框架方案落地，全球最知名的有SpringcCloud NetFlix和 SpringCloud Alibaba。</p><p>现在已经把应用模块分成了好几个，但互相调用比较麻烦，所以在此基础上，可以创建一个服务注册中心，专门用来管理各个模块。 同时每个模块也需要有各自的配置信息，模块微服务要调用后台的开发环境或测试环境，就要修改配置文件，比较麻烦，所以我们需要创建一个配置中心,一处修改，多处调用 <img src="'+l+'" alt=""></p><p>完成微服务方案需要很多组件一起来完成，这些组件要依赖springboot，要不然配置比较麻烦 <img src="'+t+`" alt=""></p><p>各个模块区分了，如果你要调用另一个模块，就要像浏览器那样发送http请求</p><p>以前用的是httpClient，用起来虽然比较方便，但是代码多，这里我们用另一个</p><p>RestTemplate（spring-web内部已经提供了，不需要单独导入）</p><p>我们需要把他定义到springioc容器中，因为启动类就是配置类，我们a要调用b，所以就在a的启动类中定义@Bean注解，把RestTemplate放入springioc容器中\\</p><div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark" style="--shiki-light:#24292e;--shiki-dark:#e1e4e8;--shiki-light-bg:#fff;--shiki-dark-bg:#24292e;" tabindex="0" dir="ltr"><code><span class="line"><span>@LoadBalanced // 开启负载均衡</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h4 id="问题-如果有多个过滤器-谁先执行-谁后执行" tabindex="-1">问题：如果有多个过滤器，谁先执行，谁后执行？ <a class="header-anchor" href="#问题-如果有多个过滤器-谁先执行-谁后执行" aria-label="Permalink to “问题：如果有多个过滤器，谁先执行，谁后执行？”">​</a></h4><p>可以实现Ordered接口，实现里面的方法，数值越低，优先级越高！ 如果是负数，肯定第一个执行。</p>`,130)])])}const m=a(d,[["render",o]]);export{y as __pageData,m as default};
